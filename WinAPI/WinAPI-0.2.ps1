### ©2023 Lifailon
### Simple REST API/Web server for Kinozal-Bot (https://github.com/Lifailon/Kinozal-Bot)
...
function Get-SerciceDescription {
    param (
        $ServiceName
    )
...
$BodyButtons += "<button onclick='location.href=""/api/network""'>Network</button>"
...
function Get-CPU {
    [string]((Get-CimInstance Win32_PerfFormattedData_PerfOS_Processor -ErrorAction Ignore | Where-Object name -eq "_Total").PercentProcessorTime)+" %"
}
    ...
    while ($http.IsListening) {
        ...
        if ( $CredRequest -ne $cred ) {
            ...
            elseif ($context.Request.HttpMethod -eq "GET" -and $context.Request.RawUrl -eq "/service") {
                $Services = Get-SerciceDescription *
                $GetService = "<html><head><title>Service</title></head><body>"
                ### Add button to process page
                $GetService += "
                <button onclick='location.href=""/process""'>Process</button> 
                <button onclick='location.href=""/api/hardware""'>Hardware</button><br><br>
                "
            ...
            elseif ($context.Request.HttpMethod -eq "GET" -and $context.Request.RawUrl -eq "/api/service") {
                $GetService = Get-SerciceDescription *
            ...
            elseif ($context.Request.HttpMethod -eq "GET" -and $context.Request.RawUrl -match "/api/service/.") {
                ...
                $GetService = Get-SerciceDescription *$ServiceName*
            ...
            elseif ($context.Request.HttpMethod -eq "GET" -and $context.Request.RawUrl -eq "/process") {
                ...
                $GetProcess += "
                <button onclick='location.href=""/service""'>Service</button> 
                <button onclick='location.href=""/api/hardware""'>Hardware</button><br><br>
                "
                ### Add form input text
                $GetProcess += "
